...
# Standalone rules to avoid security group cycle
...
resource "aws_secretsmanager_secret_version" "app" {
  ...
  secret_string = jsonencode({
    DATABASE_URL = "postgresql://appuser:${var.db_password}@${aws_db_instance.postgres.address}:5432/appdb?sslmode=require"
    DB_SSL       = "true"
    NODE_ENV     = "production"
  })
}
  ...
  container_definitions = jsonencode([
    {
      ...
      secrets = [
        ...
        {
          name      = "DB_SSL"
          valueFrom = "${aws_secretsmanager_secret.app.arn}:DB_SSL::"
        },
        {
          name      = "NODE_ENV"
          valueFrom = "${aws_secretsmanager_secret.app.arn}:NODE_ENV::"
        }
      ]
